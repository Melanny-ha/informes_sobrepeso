...
file_path_Env = r"G:\.shortcut-targets-by-id\1rqpfbdZ6z51epFv6ZwognhckW7HqBMjN\SIRI_2024_INFORMES_SOBREPESO\INFORME_ENVASE_SOLUBLE_MEDELLIN\Consolidado.xlsx"
#df = pd.read_excel(file_path, sheet_name="Hoja1")  # Cambia "Hoja1" si es necesario

# ===========================================================================
# Funciones
# ===========================================================================
#Función para convertir DataFrame a Excel(Descargar tabla)
...
# Funcion reducir decimales visualmente
...
if not os.path.exists(file_path_Env):
    st.error("No se encontró el archivo de datos en la ruta especificada.")
else:
    #Leer el archivo Excel con pandas
    df = pd.read_excel(file_path_Env, sheet_name="Hoja1")

    #Convertir los nombres de columnas a str y eliminar espacios adicionales
    df.columns = df.columns.astype(str).str.strip()

    #Renombrar las columnas utilizadas
    df.rename(columns={'Mes:':'Mes', 'Año:':'Año', 'Máquina / Equipo:':'Máquina / Equipo', 'Unidades Producidas (Conformes) :':'Unidades Producidas', 
    ...
    # Definir la fecha por defecto (01/01/2025)
    ...
    fecha_fin = st.sidebar.date_input("Fecha fin:", min_value=fecha_inicio, max_value=max_fecha) #Fecha fin no puede ser menor a fecha inicio

    #se crea una copia del dataframe para filtrar datos
    df_filtrado = df.copy()

    #Filtrar los datos en base a la fecha seleccionada
    if 'Fecha' in df_filtrado.columns:
        df_filtrado = df_filtrado[(df_filtrado['Fecha'] >= pd.to_datetime(fecha_inicio)) & (df_filtrado['Fecha'] <= pd.to_datetime(fecha_fin))] #muestra los datos entre el rango de fecha seleccionado

    #Filtrar solo las máquinas disponibles en ese rango de fechas filtrado
    maquinas_disponibles = df_filtrado['Máquina / Equipo'].unique()

    # Verificar si hay máquinas disponibles después del filtro
    if len(maquinas_disponibles) > 0:
        #Listado de maquinas solo con las máquinas filtradas
        maquina_seleccionada = st.sidebar.multiselect(
            "Seleccione Máquina/Equipo:", 
            maquinas_disponibles
        )
    else:
        #Error si no hay máquinas en ese rango
        st.info("No hay máquinas disponibles en este rango de fechas.")
    
    #Filtrar los datos en base a la maquina seleccionada
    if maquina_seleccionada:
        df_filtrado = df_filtrado[df_filtrado['Máquina / Equipo'].isin(maquina_seleccionada)]

    #Filtrar solo los turnos disponibles en ese rango de fechas filtrado
    turnos_disponibles = df_filtrado['Turno:'].unique()

    # Verificar si hay turnos disponibles después del filtro
    if len(turnos_disponibles) > 0:
        #Listado de turnos solo con los turnos filtrados
        turnos_disponibles = st.sidebar.multiselect(
            "Seleccione Turno:", 
            turnos_disponibles
        )
    else:
        #Error si no hay turnos en ese rango
        st.info("No hay turnos disponibles en este rango de fechas y/o para esta(s) máquina(s).")
    
    #Filtrar los datos en base al turno seleccionado
    if turnos_disponibles:
        df_filtrado = df_filtrado[df_filtrado['Turno:'].isin(turnos_disponibles)]

    #Filtro de codigo en base a la maquina seleccionada
    #codigo_disponible = df_filtrado['Codigo'].unique() if not df_filtrado.empty else [] #sino
    codigo_disponible = df_filtrado['Codigo'].unique()

        # Verificar si hay máquinas disponibles después del filtro
    if len(codigo_disponible) > 0:
        #Listado de codigos en base a la maquina seleccionada
        codigo_seleccionado = st.sidebar.multiselect("Seleccione Código:", codigo_disponible)
    else:
        #Error si no hay codigo en los filtros
        st.info("No hay codigos disponibles enlazados a esta máquina y/o turno(s).")
        codigo_disponible = []   #si esta vacio no muestra nada

    #Filtrar los datos en base al codigo seleccionado
    if codigo_seleccionado:
        df_filtrado = df_filtrado[df_filtrado['Codigo'].isin(codigo_seleccionado)]

    #Crear tabla pivote incluyendo Codigo y Máquina y Mes_N el ultimo para ordenar
    df_Env_MES = df_filtrado.pivot_table(
        index=['Año', 'Mes', 'Mes_N', 'Codigo', 'Máquina / Equipo'],
    ...
    df_Env_MES['Cantidad Real Empacada [kg]'] = df_Env_MES['Unidades Producidas'] * df_Env_MES['Peso Promedio de la unidad (K):']
    df_Env_MES['Cantidad Teórica a Empacar [kg]'] = df_Env_MES['Gramaje (K):'] * df_Env_MES['Unidades Producidas']
    df_Env_MES['Diferencia [kg]'] = df_Env_MES['Cantidad Real Empacada [kg]'] - df_Env_MES['Cantidad Teórica a Empacar [kg]']
    df_Env_MES['Sobrepeso [%]'] = (df_Env_MES['Diferencia [kg]'] / df_Env_MES['Cantidad Teórica a Empacar [kg]']) * 100

    #Crear tabla pivote agrupando por Año y mes, inclutendo Mes_N para ordenar
    df_Env_MES_visual = df_Env_MES.pivot_table(
        index=['Año', 'Mes', 'Mes_N'],
    ...
    df_Env_MES_visual['Sobrepeso [%]'] = (df_Env_MES_visual['Diferencia [kg]']/df_Env_MES_visual['Cantidad Teórica a Empacar [kg]']) * 100
    
    # Ordenar por Año y por Mes los dataframe
    df_Env_MES_visual = df_Env_MES_visual.sort_values(by=['Año', 'Mes_N'], ascending=[True, True])
    ...
    if df_Env_MES.empty:
        ...
        with col_izq:
            ...
            st.markdown(f'<div class="metric-card">Total Teórico [kg]<br><b>{df_Env_MES_visual['Cantidad Real Empacada [kg]'].sum():,.0f}</b></div>', unsafe_allow_html=True)
            st.markdown(f'<div class="metric-card">Total Real [kg]<br><b>{df_Env_MES_visual['Cantidad Teórica a Empacar [kg]'].sum():,.0f}</b></div>', unsafe_allow_html=True)
            st.markdown(f'<div class="metric-card">Diferencia Total [kg]<br><b>{df_Env_MES_visual['Diferencia [kg]'].sum():,.0f}</b></div>', unsafe_allow_html=True)
            sobrepeso_ratio = df_Env_MES_visual['Diferencia [kg]'].sum() / df_Env_MES_visual['Cantidad Teórica a Empacar [kg]'].sum()
            st.markdown(f'<div class="metric-card">Sobrepeso Total [%]<br><b>{sobrepeso_ratio:.2%}</b></div>', unsafe_allow_html=True)

        with col_der:
            ## ==============================================================================
            ## Tabla principal informe envase soluble
            ## ==============================================================================
            #Titulo tabla1
            st.markdown(
                """
                <div style="display: flex; justify-content: flex-end; padding: 0;">
                    <h1 style="font-size: 15px; margin: 0; color: black;">Informe Mensual de Sobrepeso - Envase Soluble</h1>
                </div>
            ...
            df_Env_MES_visual=df_Env_MES_visual[columnas]
            ...
            df_Env_MES_visual[columnas_0f] = df_Env_MES_visual[columnas_0f].apply(lambda x: x.map("{:.0f}".format))
            
            #Limitar la columna Sobrepeso a 2 decimales
            df_Env_MES_visual['Sobrepeso [%]'] = df_Env_MES_visual['Sobrepeso [%]'].apply(lambda x: f"{x:.2f}")

            #Mostrar tabla con funcion formatear
            st.markdown(f"""
                <div class="custom-table">
                    {df_Env_MES_visual.to_html(index=False)}
                </div>
            """, unsafe_allow_html=True)

            #Generar el Excel en base a la tabla visual
            excel_data = convertir_a_excel(df_Env_MES_visual)
        ...
        pivot_heatmap = df_Env_MES.pivot_table(
        ...
        with col_der:
            if 'Sobrepeso [%]' in df_Env_MES.columns:
                ## ==============================================================================
                ## Agregar histograma según código
                ## ==============================================================================
                #Eje x
                histograma = px.histogram(df_Env_MES, x='Sobrepeso [%]', nbins=25, title='Histograma de Sobrepeso')
                #Eje y
        ...
        df_ranking_prod = df_Env_MES[columnas_pareto].copy()
        ...
        if Acumulado_Real_Empaque < df_ranking_prod['[%] Acumulado'].min():
            st.info("No hay datos con dicho sobrepeso acumulado")
        else:
        ...
        df_ranking_sobre = df_Env_MES[columnas_pareto].copy()
