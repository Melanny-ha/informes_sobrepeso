...
file_path_Mol = r"G:\.shortcut-targets-by-id\1rqpfbdZ6z51epFv6ZwognhckW7HqBMjN\SIRI_2024_INFORMES_SOBREPESO\INFORME_MOLIDO\Consolidado.xlsx"
...
if not os.path.exists(file_path_Mol):
    ...
    df_Mol_MES['Cantidad Real Empacada [kg]'] = df_Mol_MES['Unidades Producidas'] * df_Mol_MES['Peso Promedio de la unidad (K):']
    df_Mol_MES['Cantidad Teórica a Empacar [kg]'] = df_Mol_MES['Gramaje (K):'] * df_Mol_MES['Unidades Producidas']
    df_Mol_MES['Diferencia [kg]'] = df_Mol_MES['Cantidad Real Empacada [kg]'] - df_Mol_MES['Cantidad Teórica a Empacar [kg]']
    ...
    if df_Mol_MES.empty:
        ...
        with col_izq:
            ...
            st.markdown(f'<div class="metric-card">Total Teórico [kg]<br><b>{df_Mol_MES_visual['Cantidad Real Empacada [kg]'].sum():,.0f}</b></div>', unsafe_allow_html=True)
            st.markdown(f'<div class="metric-card">Total Real [kg]<br><b>{df_Mol_MES_visual['Cantidad Teórica a Empacar [kg]'].sum():,.0f}</b></div>', unsafe_allow_html=True)
        ...
        with col_der:
            ...
            st.markdown(
                ...
                <div style="display: flex; justify-content: flex-end; padding: 0;">
                    <h1 style="font-size: 15px; margin: 0; color: black;">Informe Mensual de Sobrepeso - Molido</h1>
                </div>
            ...
            excel_data = convertir_a_excel(df_Mol_MES_visual)
        ...
        pivot_heatmap = df_Mol_MES.pivot_table(
        ...
        with col_der:
            if 'Sobrepeso [%]' in df_Mol_MES.columns:
                ...
                ## ==============================================================================
                #Eje x
                histograma = px.histogram(df_Mol_MES, x='Sobrepeso [%]', nbins=25, title='Histograma de Sobrepeso')
                #Eje y
